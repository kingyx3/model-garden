...
class BootstrapClientWorkspaceTests(unittest.TestCase):
    def test_bootstrap_creates_pinned_valid_compilable_workspace(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("Preferred managed cloud deployment", readme)
            self.assertIn("scripts/bootstrap-cloud.py", readme)
            self.assertIn("revoke that service-account key and delete the local JSON", readme)
            self.assertIn("self-hosted Docker-runner path remains supported", readme)
